...
*/
#define EXAMPLE_WIFI_SSID "INFINITUMD168_2.4" 
#define EXAMPLE_WIFI_PASS "fmZTVaVgNT" 
...
void aws_iot_task(void *param) {
    ...
    const char *TOPIC_SUB = "AWS/esp32_sub";
    const char *TOPIC_PUB = "AWS/esp32_pub";
    const int TOPIC_LEN = strlen(TOPIC_SUB);

    ESP_LOGI(TAG, "Subscribing...");
    rc = aws_iot_mqtt_subscribe(&client, TOPIC_SUB, TOPIC_LEN, QOS0, iot_subscribe_callback_handler, NULL);
    if(SUCCESS != rc) {
    ...
    while((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc)) {
        ...
        rc = aws_iot_mqtt_publish(&client, TOPIC_PUB, TOPIC_LEN, &paramsQOS0);

        sprintf(cPayload, "%s : %d ", "hello from ESP32 (QOS1)", i++);
        paramsQOS1.payloadLen = strlen(cPayload);
        rc = aws_iot_mqtt_publish(&client, TOPIC_PUB, TOPIC_LEN, &paramsQOS1);
